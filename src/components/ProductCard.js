...
export default function ProductCard({
  ...
  return (
      <Link to={`/product/${id}`}
        className="bg-white hover:drop-shadow-xl relative"
        onMouseEnter={() => setIsCartHidden(false)}
        ...
        <div className="flex flex-col items-center p-4">
          <div className="relative mb-4">
            <div
              className={
                outOfStock
                  ? "flex items-center justify-center bg-white opacity-50 absolute"
                  : "hidden"
              }
              style={{ height: "330px", width: "354px" }}
            >
              <p className="--font-raleway text-2xl font-normal opacity-50">
                OUT OF STOCK
              </p>
            </div>
            <img
          ...
          <div className="flex flex-col w-full">
            <p className="--font-raleway text-lg font-light">{title}</p>
            <p className="--font-raleway text-lg font-bold">{price}</p>
          </div>
        </div>
        <button
          onClick={(event) => handleClick(event, product)}
          className={
            isCartHidden || outOfStock
              ? "hidden"
              : "absolute right-8 bottom-16 drop-shadow-md"
          }
        >
          <svg
            width="52"
